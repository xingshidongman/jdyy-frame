<template lang="pug">
  ...
  keep-alive
    el-form(:model="formModel" :rules="rules" ref="formModel")
      el-form-item(label="值班医生" prop="doctor" v-bind:rules="rules.doctor" v-bind:label-width="labelWidth")
        el-input(v-model="formModel.doctor")
      el-form-item(label="值班日期" prop="date" v-bind:rules="rules.date" v-bind:label-width="labelWidth")
        kalix-datepicker-simple(v-model="formModel.date" type="datetime" placeholder="选择日期" format="yyyy-MM-dd" style="width: 100%;")
      el-form-item(label="原住院人数" prop="protoNum" v-bind:rules="rules.protoNum" v-bind:label-width="labelWidth")
        el-input(v-model="formModel.protoNum")
      el-form-item(label="出院人数" prop="outNum" v-bind:rules="rules.outNum" v-bind:label-width="labelWidth")
        el-input(v-model="formModel.outNum")
      el-form-item(label="入院（转）入人数" prop="inNum" v-bind:rules="rules.inNum" v-bind:label-width="labelWidth")
        el-input(v-model="formModel.inNum")
      el-form-item(label="手术人数" prop="surgeryNum" v-bind:rules="rules.surgeryNum" v-bind:label-width="labelWidth")
        el-input(v-model="formModel.surgeryNum")
      el-form-item(label="死亡人数" prop="deathNum" v-bind:rules="rules.deathNum" v-bind:label-width="labelWidth")
        el-input(v-model="formModel.deathNum")
      el-form-item(label="现住院人数" prop="nowNum" v-bind:rules="rules.nowNum" v-bind:label-width="labelWidth")
        el-input(v-model="formModel.nowNum")
      el-form-item(label="重病人数" prop="illNum" v-bind:rules="rules.illNum" v-bind:label-width="labelWidth")
