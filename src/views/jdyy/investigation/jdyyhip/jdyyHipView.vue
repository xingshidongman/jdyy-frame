  ...
  kalix-dialog.user-add(title='全髋术后查看' bizKey="jdyyHip" ref="kalixBizDialog" v-bind:formModel.sync="formModel" v-bind:labelWidth="labelWidth" v-bind:labelPosition="labelPosition" isView)
    div.back(slot="dialogFormSlot")
                ...
                td.long-td
                  div.con2-right
                    el-form-item.short(label="关节炎")
                      el-select(v-model="formModel.arthritis" placeholder="请选择" readonly)
                    ...
                    el-form-item.short(label="骨坏死")
                      el-select(v-model="formModel.osteonecrosis" placeholder="请选择" readonly)
                    ...
                    el-form-item.short(label="常见病因")
                      el-select(v-model="formModel.pathogeny" placeholder="请选择" readonly)
                    ...
                    el-form-item.other(label="其他")
                      el-input(v-model="formModel.other1" readonly)
              tr
                td.short-td
                  div.con2-left 翻修指征
                td.long-td
                ...
                td.long-td
                  div.con2-right
                    el-form-item.short(label="对侧髋关节")
                      el-select(v-model="formModel.contralateral" placeholder="请选择" readonly)
                ...
                td.long-td
                  div.con2-right
                    el-form-item(label="分类")
                      el-select.long(v-model="formModel.classification" placeholder="请选择" readonly)
            ...
            table
                ...
                td.long-td
                  div.con2-right
                    el-form-item.short(label="体位")
                      el-select(v-model="formModel.posture" placeholder="请选择" readonly)
                        el-option(label="侧卧" value="侧卧")
                        el-option(label="仰卧" value="仰卧")
                        el-option(label="其他" value="其他")
                    el-form-item.short(label="入路")
                      el-select(v-model="formModel.approach" placeholder="请选择" readonly)
                ...
                td.long-td
                  div.con2-right
                    el-form-item.short(label="瘢痕")
                      el-select(v-model="formModel.scar" placeholder="请选择" readonly)
                        el-option(label="微小" value="微小")
                        el-option(label="正常" value="正常")
                        el-option(label="严重" value="严重")
                    el-form-item.short(label="臼杯")
                      el-select(v-model="formModel.acetabular" placeholder="请选择" readonly)
                    ...
                    el-form-item.short(label="股骨头")
                      el-select(v-model="formModel.femoralhead" placeholder="请选择" readonly)
                        el-option(label="完整" value="完整")
                        el-option(label="磨损" value="磨损")
                    el-form-item.short(label="股骨柄")
                      el-select(v-model="formModel.femoralstem" placeholder="请选择" readonly)
                        el-option(label="完整" value="完整")
                        el-option(label="松动" value="松动")
                        el-option(label="严重松动" value="严重松动")
                    el-form-item.short(label="骨溶解")
                      el-select(v-model="formModel.osteolysis" placeholder="请选择" readonly)
                        el-option(label="无" value="无")
              ...
              tr
                ...
                td.long-td
                  div.con2-right
                    el-form-item.short(label="臼杯")
                      el-input(v-model="formModel.acetabular2" readonly)
                    el-form-item.short(label="臼杯类型")
                      el-select(v-model="formModel.acetabularType" placeholder="请选择" readonly)
                        el-option(label="水泥型" value="水泥型")
                        el-option(label="生物型" value="生物型")
                    el-form-item.short(label="股骨柄")
                      el-input(v-model="formModel.femoralstem2" readonly)
                    el-form-item.short(label="股骨柄类型")
                      el-select(v-model="formModel.femoralstemType" placeholder="请选择" readonly)
                        el-option(label="水泥型" value="水泥型")
                        el-option(label="生物型" value="生物型")
              tr
                td.short-td
                ...
                td.long-td
                  div.con2-right
                    el-form-item.short(label="球头直径")
                      el-input(v-model="formModel.ballDiameter" readonly)
                        template(slot="append") mm
                    el-form-item.short(label="材料")
                      el-select(v-model="formModel.material" placeholder="请选择" readonly)
                    ...
                    el-form-item(label="颈长")
                      el-input(v-model="formModel.diameterLength" readonly)
              tr
                td.short-td
                  div.con2-left 髋臼
                td.long-td
                  div.con2-right
                    el-form-item.short(label="髋臼磨锉")
                      el-input(v-model="formModel.aModel" readonly)
                        template(slot="append") mm
                    el-form-item.short(label="假体")
                      el-input(v-model="formModel.bModel" readonly)
                        template(slot="append") mm
                    el-form-item.short(label="加减")
                      el-input(v-model="formModel.cModel" readonly)
                        template(slot="prepend") 号=
                    el-form-item.short(label="包容")
                      el-input(v-model="formModel.Inclusiveness" readonly)
                    el-form-item.short(label="稳定性")
                      el-select(v-model="formModel.stable1" placeholder="请选择" readonly)
                    ...
                    div.mark
                      ...
                      div.stb
                        el-form-item.mini.toleft(label="外翻")
                          el-input(v-model="formModel.ectropion" readonly)
                            template(slot="append") °
                        el-form-item.mini(label="前倾")
                          el-input(v-model="formModel.forward " readonly)
                            template(slot="append") °
                    div.mark
                      div.sta 聚乙烯衬垫植入角度
                      div.stb
                        el-form-item.mini.toleft
                          el-input(v-model="formModel.inAngle" readonly)
                            template(slot="append") °
                        el-form-item.mini(label="防脱缘 在")
                          el-input(v-model="formModel.inclock " readonly)
                ...
                td.long-td
                  div.con2-right
                    el-form-item.short(label="髓腔锉型号")
                      el-input(v-model="formModel.medullaryCavity" readonly)
                    el-form-item.short(label="假体型号")
                      el-input(v-model="formModel.prosthesisType " readonly)
                    el-form-item.short(label="假体长度")
                      el-input(v-model="formModel.prosthesisLength" readonly)
                    el-form-item.short(label="远端骨水泥塞")
                      el-select(v-model="formModel.cement" placeholder="请选择" readonly)
                        el-option(label="骨" value="骨")
                        el-option(label="骨水泥" value="骨水泥")
                        el-option(label="伞" value="伞")
                        el-option(label="未应用" value="未应用")
                    el-form-item.short(label="稳定性")
                      el-select(v-model="formModel.stable2" placeholder="请选择" readonly)
                        el-option(label="差" value="差")
                        el-option(label="交界" value="交界")
                        el-option(label="好" value="好")
                        el-option(label="优" value="优")
                    el-form-item.short(label="捆绑线")
                      el-select(v-model="formModel.binding" placeholder="请选择" readonly)
                        el-option(label="无" value="无")
                        el-option(label="预防性" value="预防性")
                        el-option(label="术中存在骨折" value="术中存在骨折" readonly)
                    el-form-item.short(label="假体术中力线")
                      el-input(v-model="formModel.forceLine" readonly)
                ...
                td.long-td
                  div.con2-right
                    el-form-item.short(label="皮质骨开窗")
                      el-input(v-model="formModel.windowOpen" readonly)
                    el-form-item.short(label="股骨端截骨")
                      el-radio-group(v-model="formModel.Osteotomy" disabled)
                        el-radio(label="有")
                        el-radio(label="无")
                    el-form-item.short(label="并发症")
                      el-select(v-model="formModel.Complication" placeholder="请选择" readonly)
                        el-option(label="无" value="无")
                    ...
                    el-form-item.short(label="股骨缺陷")
                      el-select(v-model="formModel.defect1" placeholder="请选择" readonly)
                        el-option(label="无" value="无")
                    ...
                    div.mark
                      ...
                      div.stb
                        el-form-item.long.toleft(label="中心型")
                          el-input(v-model="formModel.centerType" readonly)
                        el-form-item.long.toleft(label="髋臼周边")
                          el-input(v-model="formModel.periphery " readonly)
                    div.mark
                      div.sta 髋臼植骨
                      div.stb
                ...
                td.long-td
                  div.con2-right
                    div.mark
                      ...
                      div.stb
                        el-form-item.mini.toleft
                          el-input(v-model="formModel.frontA" readonly)
                            template(slot="append") °屈曲
                        el-form-item.mini.toleft
                          el-input(v-model="formModel.frontB " readonly)
                            template(slot="append") °内收
                        el-form-item.mini.toleft
                          el-input(v-model="formModel.frontC " readonly)
                            template(slot="append") °内旋
                    div.mark
                      div.sta 前方稳定性
                      div.stb
                        el-form-item.mini.toleft
                          el-input(v-model="formModel.afterA" readonly)
                            template(slot="append") °伸展
                        el-form-item.mini.toleft
                          el-input(v-model="formModel.afterB " readonly)
                      ...
                      div.stb
                        el-form-item.toleft
                          el-radio-group(v-model="formModel.resection" disabled)
                            el-radio(label="有")
                            el-radio(label="无")
                        template(v-if="formModel.resection === '无'" readonly)
                          el-form-item.mini(label="外展" )
                            el-input(v-model="formModel.resectionAbduction" readonly)
                              template(slot="append") °
                          el-form-item.mini(label="增加角度" )
                            el-input(v-model="formModel.resectionaAngle" readonly)
                              template(slot="append") °
              tr
                td.short-td
                  div.con2-left 术后期
                td.long-td
                  div.con2-right
                    el-form-item.short(label="术后总引流量")
                      el-input(v-model="formModel.flow" readonly)
                      ...
                      div.stb
                        el-form-item.toleft
                          el-radio-group(v-model="formModel.singleAbduction" disabled)
                            el-radio(label="无法使用")
                            el-radio(label="可以使用")
                        template(v-if="formModel.singleAbduction === '可以使用'" readonly)
                          el-form-item.mini(label="术后" )
                            el-input(v-model="formModel.singleDay" readonly)
                              template(slot="append") 天
                    div.mark
                      div.sta 出院
                      div.stb
                        el-form-item.mini.toleft(label="术后" )
                          el-input(v-model="formModel.leaveHospital" readonly)
                            template(slot="append") 天
                    div.mark
                      div.sta 出院时关节活动角度
                      div.stb
                        el-form-item.mini.toleft(label="屈曲")
                          el-input(v-model="formModel.angleA" readonly)
                            template(slot="append") °
                        el-form-item.mini.toleft(label="外展")
                          el-input(v-model="formModel.angleB " readonly)
                            template(slot="append") °
                        el-form-item.mini.toleft(label="内旋")
                          el-input(v-model="formModel.angleC " readonly)
                            template(slot="append") °
                        el-form-item.mini.toleft(label="过伸")
                          el-input(v-model="formModel.angleD " readonly)
                            template(slot="append") °
                        el-form-item.mini.toleft(label="内收")
                          el-input(v-model="formModel.angleE " readonly)
                            template(slot="append") °
                        el-form-item.mini.toleft(label="外旋")
                          el-input(v-model="formModel.angleF " readonly)
                            template(slot="append") °
        div.art3
          h1.title 术后影像学检查
          div.con3
            table
                ...
                td.b-td
                  div.con3-mid
                    ...
                    el-form-item.mini(label="术后第")
                      el-input(v-model="formModel.weekDay" readonly)
                ...
                td.c-td
                  div.con3-right
                    el-form-item.totoleft
                      el-input(v-model="formModel.cupPosition" readonly)
              tr
                td.a-td
                  div.con3-left 内壁
                td.b-td
                ...
                td.c-td
                  div.con3-right
                    el-form-item.totoleft
                      el-input(v-model="formModel.wall" readonly)
              tr
                td.a-td
                  div.con3-left 臼杯外翻
                td.b-td
                ...
                td.c-td
                  div.con3-right
                    el-form-item.totoleft
                      el-input(v-model="formModel.ectropion2" readonly)
              tr
                td.a-td
                  div.con3-left 假体柄力线
                td.b-td
              ...
              tr
                ...
                td.c-td
                  div.con3-right
                    el-form-item.totoleft
                      el-input(v-model="formModel.joint" readonly)
        div.art3
          h1.title 术后并发症
          div.con3
            table
              tr
                ...
                td.c-td
                  div.con3-right
                    el-form-item.totoleft
                      el-input(v-model="formModel.orthopedics" readonly)
              tr
                td.a-td
                  div.con3-left 综合部分
                td.b-td
                ...
                td.c-td
                  div.con3-right
                    el-form-item.totoleft
                      el-input(v-model="formModel.comprehensive" readonly)
        div.art2
        ...
        div.art2
          ...
          div.con2
            el-form-item.mini(label="日期")
              el-input(v-model="formModel.harADate" readonly)
            el-form-item.mini(label="术后第（）月")
              el-input(v-model="formModel.harA1" readonly)
            el-form-item.mini(label="疼痛")
              el-input(v-model="formModel.harA2" readonly)
            el-form-item.mini(label="上下楼梯")
              el-input(v-model="formModel.harA3" readonly)
            el-form-item.mini(label="坐")
              el-input(v-model="formModel.harA4" readonly)
            el-form-item.mini(label="穿鞋袜")
              el-input(v-model="formModel.harA5" readonly)
            el-form-item.mini(label="上公交车")
              el-input(v-model="formModel.harA6" readonly)
            el-form-item.mini(label="跛行")
              el-input(v-model="formModel.harA7" readonly)
            el-form-item.mini(label="行走辅助")
              el-input(v-model="formModel.harA8" readonly)
            el-form-item.mini(label="行走距离")
              el-input(v-model="formModel.harA9" readonly)
            el-form-item.mini(label="无畸形")
              el-input(v-model="formModel.harA10" readonly)
            el-form-item.mini(label="关节活动度")
              el-input(v-model="formModel.harA11" readonly)
            div.mark
              div.sta 总分：{{total1}}
        ...
        div.art2
          ...
          div.con2
            el-form-item.mini(label="大腿痛")
              el-input(v-model="formModel.harBDate" readonly)
            el-form-item.elin(label="Trendelenburg 征" label-width="150px")
              el-input(v-model="formModel.harB1" readonly)
            el-form-item.mini(label="活动度-伸直")
              el-input(v-model="formModel.harB2" readonly)
            el-form-item.mini(label="活动度-屈曲")
              el-input(v-model="formModel.harB3" readonly)
            el-form-item.mini(label="活动度-外展")
              el-input(v-model="formModel.harB4" readonly)
            el-form-item.mini(label="活动度-内收")
              el-input(v-model="formModel.harB5" readonly)
            el-form-item.mini(label="活动度-内旋")
              el-input(v-model="formModel.harB6" readonly)
            el-form-item.mini(label="活动度-外旋")
              el-input(v-model="formModel.harB7" readonly)
            el-form-item.mini(label="双下肢不等长")
              el-input(v-model="formModel.harB8" readonly)
        div.art2
          h1.title 患者满意度
          div.con2
            table
          ...
          div.con2
            el-form-item.mini(label="日期")
              el-input(v-model="formModel.harCDate" readonly)
            el-form-item.mini(label="与术前相比")
              el-input(v-model="formModel.harC1" readonly)
            el-form-item.mini(label="术后第（）月")
              el-input(v-model="formModel.harC2" readonly)
            el-form-item.mini(label="疼痛")
              el-input(v-model="formModel.harC3" readonly)
            el-form-item.mini(label="功能")
              el-input(v-model="formModel.harC4" readonly)
            el-form-item.mini(label="总评")
              el-input(v-model="formModel.harC5" readonly)
        div.art2
          h1.title 影像学评估
          div.con2
            table
        ...
        div.art2
          ...
          div.con2
            el-form-item.mini(label="日期")
              el-input(v-model="formModel.harDDate" readonly)
            el-form-item.elin(label="术后第（）月" label-width="150px")
              el-input(v-model="formModel.harD1" readonly )
            el-form-item.mini(label="移位")
              el-input(v-model="formModel.harD2" readonly)
            el-form-item.mini(label="聚乙烯磨损")
              el-input(v-model="formModel.harD3" readonly)
            el-form-item.elin(label="骨水泥透亮区 -1 *" label-width="150px")
              el-input(v-model="formModel.harD4" readonly)
            el-form-item.elin(label="骨水泥透亮区 -2 *" label-width="150px")
              el-input(v-model="formModel.harD5" readonly)
            el-form-item.mini(label="骨溶解 *")
              el-input(v-model="formModel.harD6" readonly)
            el-form-item.mini(label="增生*")
              el-input(v-model="formModel.harD7" readonly)
            el-form-item.mini(label="萎缩 *")
              el-input(v-model="formModel.harD8" readonly)
            el-form-item.mini(label="异位骨化")
              el-input(v-model="formModel.harD9" readonly)
            el-form-item.mini(label="金属问题")
              el-input(v-model="formModel.harD10" readonly)
            el-form-item.elin(label="骨水泥金属界面透亮区 *" label-width="190px")
              el-input(v-model="formModel.harD11" readonly)
            el-form-item.elin(label="骨水泥骨折（） *" label-width="150px")
              el-input(v-model="formModel.harD12" readonly)
            el-form-item.mini(label="涂层脱落*")
              el-input(v-model="formModel.harD13" readonly)
</template>
